...
void main(int argc, char **argv)
{
	if (argc != 3)
	{
		printf("Usage: tool <source filename> <destination filename>\n");
		exit(0);
	}
	...
	int dataSize = ftell(srcFile);
	fseek(srcFile, 0, SEEK_SET);

	// read file
	char *srcData = new char[dataSize];
	fread(srcData, dataSize, 1, srcFile);
	fclose(srcFile);
	...
	unsigned index = dataSize - SKIP_SIZE;
	index = index * 244109 + 240169;

	for (int i = SKIP_SIZE; i < dataSize; i++)
	{
		index = index % 761;
		srcData[i] ^= table[index];
		index++;
	}

	fwrite(srcData + SKIP_SIZE, dataSize - SKIP_SIZE, 1, dstFile);
	fclose(dstFile);
}
